...
internal sealed interface GooglePart {
    ...
    /** Code generated by Gemini for execution in its provider-managed Python runtime. */
    @Serializable
    @SerialName("executableCode")
    data class ExecutableCode(
        val executableCode: GoogleData.ExecutableCode,
        override val thought: Boolean? = null,
        override val thoughtSignature: String? = null,
    ) : GooglePart

    /** Output returned by Gemini's provider-managed code execution runtime. */
    @Serializable
    @SerialName("codeExecutionResult")
    data class CodeExecutionResult(
        val codeExecutionResult: GoogleData.CodeExecutionResult,
        override val thought: Boolean? = null,
        override val thoughtSignature: String? = null,
    ) : GooglePart
}
...
internal sealed interface GoogleData {
    ...
    class ExecutableCode(
        val id: String? = null,
        val language: GoogleCodeExecutionLanguage,
        val code: String,
    ) : GoogleData

    /** Result emitted for an [ExecutableCode]. */
    @Serializable
    class CodeExecutionResult(
        val id: String? = null,
        val outcome: GoogleCodeExecutionOutcome,
        val output: String = "",
    ) : GoogleData
}

/** Languages accepted by Gemini's hosted execution runtime. */
@Serializable
internal enum class GoogleCodeExecutionLanguage {
    @SerialName("LANGUAGE_UNSPECIFIED")
...
/** Terminal outcomes returned by Gemini's hosted execution runtime. */
@Serializable
internal enum class GoogleCodeExecutionOutcome {
    @SerialName("OUTCOME_UNSPECIFIED")
    OUTCOME_UNSPECIFIED,

    @SerialName("OUTCOME_OK")
    OUTCOME_OK,

    @SerialName("OUTCOME_FAILED")
    OUTCOME_FAILED,

    @SerialName("OUTCOME_DEADLINE_EXCEEDED")
 ...
 * the next model turn.
 * @property codeExecution Enables provider-hosted code execution when present.
 */
@Serializable
...
/** Empty configuration object that enables Gemini hosted code execution. */
@Serializable
internal class GoogleCodeExecutionTool
...
internal object GooglePartSerializer : JsonContentPolymorphicSerializer<GooglePart>(GooglePart::class) {
    override fun selectDeserializer(element: JsonElement): DeserializationStrategy<GooglePart> {
        ...
        return when {
            ...
            has("executableCode") -> GooglePart.ExecutableCode.serializer()
            has("codeExecutionResult") -> GooglePart.CodeExecutionResult.serializer()

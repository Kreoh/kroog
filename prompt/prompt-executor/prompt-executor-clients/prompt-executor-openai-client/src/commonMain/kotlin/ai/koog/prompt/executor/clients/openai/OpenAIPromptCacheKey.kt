...
/** Derives opaque OpenAI prompt-cache keys without exposing application identity to the provider. */
internal object OpenAIPromptCacheKey {
    ...
    fun derive(
        ...
        model: String,
        identity: OpenAIPromptCacheIdentity,
    ): String {
    ...
    private fun MutableList<Byte>.appendField(value: String) {
        val bytes = value.encodeToByteArray()
        add((bytes.size ushr 24).toByte())
        add((bytes.size ushr 16).toByte())
        add((bytes.size ushr 8).toByte())
    ...
    private fun sha256(input: ByteArray): ByteArray {
        ...
        for (chunkStart in padded.indices step 64) {
            ...
            repeat(64) { index ->
                ...
                val temporary1 = h + sum1 + choice + RoundConstants[index] + schedule[index]
